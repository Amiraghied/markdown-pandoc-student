...
![figure 1. pic](img/rio-grande-basin.png)
...
This database has `r nrow(usgs)` sampling sites

```{r plot, echo=TRUE}

plot(faithful,main="Do I wait longer for longer eruption? ")
```
...
#  Policies for Access and Sharing
  ...
  a. The data are public and will be obtainable thru the New Mexico Interstate Stream Commission (NMISC).
  b. Users of the data will primarily be water resource managers in the Rio Grande Basin. 
  c. USGS publications will be released describing the methods and data sources and can be used as documentation for the data and to cite the data.
...
with University/Participating institutional and NSF policies.[for example , @Clement_2013], Depending
...
1. The database will be preserved in perpetuity, housed initially at the New Mexico Interstate Stream Commission Central Office in addition to an off-site copy maintained at
an NMISC field office and mirrored at the Consortium of Universities for
the Advancement of Hydrologic Science (CUAHSI).
1. We will also identify
appropriate archiving institutions that might serve as a mirror
repository. 
1. A data policy and stewardship plan will be established. In
...
software programs. 
1. The data manager at the NMISC will be responsible for
